...
func TestScrapeRequestsTotal(t *testing.T) {
	...
	t.Run("increments on failed connection", func(t *testing.T) {
		...
		before := testutil.ToFloat64(fetchDiagnosticCounters(target).requests)

		w := httptest.NewRecorder()
		metricsHandler()(w, newScrapeRequest(target, nil))

		assert.Equal(t, http.StatusInternalServerError, w.Code)
		assert.Equal(t, before+1, testutil.ToFloat64(fetchDiagnosticCounters(target).requests))
	})
	...
	t.Run("counts are per target", func(t *testing.T) {
		...
		beforeA := testutil.ToFloat64(fetchDiagnosticCounters(targetA).requests)
		beforeB := testutil.ToFloat64(fetchDiagnosticCounters(targetB).requests)
		...
		assert.Equal(t, beforeA+2, testutil.ToFloat64(fetchDiagnosticCounters(targetA).requests))
		assert.Equal(t, beforeB+1, testutil.ToFloat64(fetchDiagnosticCounters(targetB).requests))
	})
	...
	t.Run("does not increment when scrape request missing from context", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusInternalServerError, w.Code)
		assert.Equal(t, before, testutil.ToFloat64(fetchDiagnosticCounters(target).requests))
	})
}
...
func TestScrapeSuccessesTotal(t *testing.T) {
	...
	t.Run("increments when all collectors succeed", func(t *testing.T) {
		...
		target := rfServer.Listener.Addr().String()
		before := testutil.ToFloat64(fetchDiagnosticCounters(target).successes)

		metricsHandler()(httptest.NewRecorder(), newScrapeRequest(target, nil))

		assert.Equal(t, before+1, testutil.ToFloat64(fetchDiagnosticCounters(target).successes))
	})
	...
	t.Run("does not increment when connection fails", func(t *testing.T) {
		...
		assert.Equal(t, before, testutil.ToFloat64(fetchDiagnosticCounters(target).successes))
	})
	...
	t.Run("requests and successes counters are independent", func(t *testing.T) {
		...
		beforeReqs := testutil.ToFloat64(fetchDiagnosticCounters(target).requests)
		beforeSucc := testutil.ToFloat64(fetchDiagnosticCounters(target).successes)

		metricsHandler()(httptest.NewRecorder(), newScrapeRequest(target, nil))

		assert.Equal(t, beforeReqs+1, testutil.ToFloat64(fetchDiagnosticCounters(target).requests))
		assert.Equal(t, beforeSucc, testutil.ToFloat64(fetchDiagnosticCounters(target).successes))
	})
}
