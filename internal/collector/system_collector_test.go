...
// TestDuplicateDriveListingGathers reproduces BMCs (e.g. Supermicro
// ASG-2115S) that list the same drive more than once under a single storage
// controller. Without dedupe, the duplicate label sets fail the entire
// registry gather ("was collected before with the same name and label
// values"), which surfaced in production as scrape-wide HTTP 500s.
func TestDuplicateDriveListingGathers(t *testing.T) {
	srv := newTestRedfishServer(t)

	systemPath := "/redfish/v1/Systems/System1"
	storagePath := systemPath + "/Storage/NVMeSSD"
	drivePath := storagePath + "/Drives/Disk.Bay.0"
